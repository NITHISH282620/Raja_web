...
| **Edge cache purge does not fire on publish** — confirmed 2026-09-16, real end-to-end test (admin login, `saveHero()` edit, Neon verified, repeated checks) | An admin save is correct in Neon immediately, but the public edge keeps serving the previous cached HTML for up to the TTL, not instantly | `revalidatePath() updates the application/data cache path but does not currently invalidate the existing Cloudflare edge cache entry in this deployment.` TTL kept bounded (`s-maxage=3600`) specifically because of this; do not raise it or assume instant publish-to-live until the purge path is separately investigated and verified end-to-end |
