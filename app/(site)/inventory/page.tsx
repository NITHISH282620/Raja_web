...
import { getInventoryTiles } from "@/lib/store";
...
export default function InventoryPage() {
  return (
    <main id="main">
      ...
      <Band tone="ink">
        <div className="frame">
          <p className="t-eyebrow mb-[clamp(20px,2.4vw,34px)] text-ink/50">The inventory schedule</p>
          <dl className="flex flex-col">
            {inventorySchedule.map((line) => (
              <div
                key={line.item}
                data-band-item
                className="grid grid-cols-[1fr_auto] items-baseline gap-6 border-t border-ink/12 py-[clamp(14px,1.7vw,24px)] last:border-b"
              >
                <dt className="t-stat-label text-ink">{line.item}</dt>
                <dd className="m-0 text-right">
                  {line.capacity ? (
                    <span className="t-work text-ink">
                      {line.capacity}
                      {line.unit && <span className="t-body ml-2 text-body-light">{line.unit}</span>}
                    </span>
                  ) : (
                    <span className="t-eyebrow text-ink/35">In fleet</span>
                  )}
                </dd>
              </div>
            ))}
          </dl>
          <p className="t-body-sm mt-[clamp(18px,2vw,28px)] max-w-[62ch] text-body-light">
      ...
      <Band tone="ink">
        <div className="frame">
          <p className="t-eyebrow mb-[clamp(20px,2.4vw,34px)] text-ink/50">Where it goes to work</p>
          <ul className="grid gap-x-10 gap-y-[clamp(10px,1.4vw,18px)] sm:grid-cols-2 lg:grid-cols-3">
            {services.map((s) => (
              <li key={s} data-band-item className="t-work border-t border-ink/12 pt-4 text-ink">
                {s}
              </li>
            ))}
          </ul>
