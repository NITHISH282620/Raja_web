...
export default function PortfolioPage() {
  ...
  return (
    <main id="main">
        ...
        <Band>
          <div className="frame">
            <article data-band-item className="flex flex-col gap-[clamp(20px,3vw,40px)]">
              {lead.hero && (
                <div className="relative aspect-[16/8] w-full overflow-hidden rounded-[20px] bg-mist">
                  <Image
                    src={lead.hero.src}
                    alt={lead.hero.alt}
                    fill
      ...
      <Band tone="ink">
        <div className="frame">
          <p className="t-eyebrow mb-[clamp(20px,2.4vw,34px)] text-ink/50">Selected programmes</p>
              ...
              <li key={p.id} data-band-item className="flex flex-col gap-4">
                <div
                  className={clsx(
                    ...
                    p.hero ? "bg-mist" : "border border-dashed border-ink/15 bg-ink/[0.03]",
                  )}
                >
                  {p.hero ? (
                    <Image
                    ...
                    <span className="absolute inset-0 grid place-items-center">
                      <span className="t-eyebrow text-ink/35">Photography pending</span>
                    </span>
                  )}
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <h3 className="t-tile text-balance text-ink">{p.title}</h3>
                  <p className="t-body-sm text-body-light">{p.organization}</p>
                </div>
              </li>
            ))}
          </ul>

          <p className="t-body-sm mt-[clamp(28px,3.4vw,48px)] max-w-[62ch] text-body-light">
